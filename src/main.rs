...
async fn wgpu_test() {
    let mut window = Window::new("Pregen: Runtime", 800, 800, true, WindowBackend::Win32);
    ...
    let shader = context.create_shader("shaders/test.wgsl");
    dbg!(&shader);
    ...
    let pipeline = context.device.create_render_pipeline(&wg::RenderPipelineDescriptor {
        ...
        layout: Some(&context.device.create_pipeline_layout(&wg::PipelineLayoutDescriptor {
            label: None,
            push_constant_ranges: &[],
            bind_group_layouts: &[&frame_bind.layout, &camera.group.layout, &model.meshes[0].material.group.layout, &model.group.layout],
        })),
        vertex: wg::VertexState {
            module: &shader.as_raw(),
            entry_point: Some(shader.get_entry(ShaderStage::Vertex).unwrap()),
    ...
    while !window.should_close() {
        ...
        // Object rendering
        {
            ...
            rpass.set_bind_group(3, model.group.as_raw(), &[]);
            rpass.set_bind_group(1, camera.group.as_raw(), &[]);
            for mesh in &model.meshes {
                rpass.set_bind_group(2, mesh.material.group.as_raw(), &[]);
