...
fn main() {
    ...
    let profile = env::var("PROFILE").expect("Profile not found"); // FIXME: this doesnt work for custom profiles
    ...
    if !dxc_archive.exists() {
        ...
        println!("DXC downloaded successfully to {}", dxc_archive.display());
    }
    if !dxc_source.exists() {
        ...
        println!("DXC extracted successfully to {}", dxc_source.display());
    }
    if !app_bin_dir.join("dxcompiler.dll").exists() || !app_bin_dir.join("dxil.dll").exists() {
        println!("Copying DXC DLLs to {}", app_bin_dir.display());
